...
(function () {
  ...
  window.addEventListener('load', function () {
    ...
    // ── 8. إخفاء عرض كلمة المرور في الـ UI ──────────
    ...
    new MutationObserver(function (mutations) {
      mutations.forEach(function (m) {
        m.addedNodes.forEach(function (node) {
          ...
          (node.querySelectorAll ? node.querySelectorAll('*') : []).forEach(function (el) {
            if (el.dataset.masked) return;
            if (el.children.length === 0 &&
                el.parentElement &&
                el.parentElement.textContent.includes('باسورد:')) {
              el.parentElement.style.display = 'none';
              el.parentElement.dataset.masked = '1';
            }
          });
        });
      });
